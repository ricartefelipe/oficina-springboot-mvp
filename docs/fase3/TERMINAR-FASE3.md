...
| F | **PDF único** no portal (4 links + vídeo + **soat-architecture**) | **PDF** com base em [`entrega-portal-fase3.md`](entrega-portal-fase3.md). **Vídeo** pode ser omitido; usa **evidências substitutas** (links CI, prints) na secção 6 desse documento. |
...
2. **Terraform BD**: se ainda só fizeste **plan**, decide **sozinho** se fazes **`apply`** (RDS custa dinheiro).  
...
6. **PDF único** (sem vídeo, se quiseres): preenche **[`entrega-portal-fase3.md`](entrega-portal-fase3.md)** — links dos 4 repos, runs do Actions, `soat-architecture`, evidências da secção 6.  
7. **Vídeo** (opcional): só se o professor exigir; caso contrário, as evidências na secção 6 substituem a demonstração gravada.

---

## 3) Modo individual (sem equipa)

- **Não precisas** de vídeo para “provar” infra: usa **links de CI** + **descrição do Terraform** + **prints** no PDF (modelo em [`entrega-portal-fase3.md`](entrega-portal-fase3.md)).  
- **Decisão de custo** na AWS (`apply`) é **tua** (RDS, cluster).  
- O **upload do PDF** no portal é **manual**; o conteúdo do PDF prepara-se a partir do ficheiro acima.
...
**Resumo:** “Terminar a Fase 3” = **infra demonstrável** (código + CI + plan/apply conforme possas) + **documentação** + **PDF** (modelo `entrega-portal-fase3.md`). **Vídeo** opcional; evidências **substitutas** no próprio PDF.
